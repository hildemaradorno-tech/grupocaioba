...
export default function TiposAcaoAuditoria() {
  ...
  const loadDados = async () => {
    ...
    try {
      setDados(await apiService.getAuditExtTiposAcao())
    } catch (err) {
  ...
  const handleSalvar = async (e) => {
    e.preventDefault()
    try {
      if (editingId) await apiService.updateAuditExtTipoAcao(editingId, form)
      else await apiService.createAuditExtTipoAcao(form)
      await loadDados()
      setModalAberto(false)
    } catch (err) {
      alert('Erro ao salvar tipo de ação: ' + (err.message || String(err)))
    }
  }
  ...
  const handleConfirmarExclusao = async () => {
    try {
      ...
      alert('Erro ao excluir tipo de ação: ' + (err.message || String(err)))
    } finally {
  ...
  if (error) return (
    <div className="p-6">
      <div className="bg-yellow-50 border border-yellow-200 rounded p-6">
        <h2 className="text-lg font-semibold mb-2">Erro ao carregar tipos de ação</h2>
    ...
    <div className="p-6 space-y-4 max-w-screen-md">
      <div className="space-y-3 border-b border-slate-200 pb-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-slate-900 tracking-tight">Tipos de Ação</h1>
            <p className="text-xs text-slate-500">Cadastro do tipo de ação tomada, usado no Plano de Ação.</p>
          </div>
          {canEdit && (
            <button onClick={abrirIncluir} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold px-3 py-2 rounded-md shadow-sm transition-colors">
              <Plus className="h-4 w-4" /> Novo Tipo de Ação
            </button>
          )}
        </div>
        <AuditoriaExternaNav />
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
        <table className="w-full table-auto text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-700">
            {dados.length === 0 ? (
              <tr><td colSpan="3" className="p-6 text-center text-slate-400">Nenhum tipo de ação cadastrado.</td></tr>
            ) : dados.map(item => (
              <tr key={item.id} className="hover:bg-slate-50/70 transition-colors">
                <td className="p-3 text-slate-900 font-bold flex items-center gap-2"><Tag className="h-3.5 w-3.5 text-indigo-500" /> {item.nome}</td>
                <td className="p-3">
        ...
        <div className="fixed top-0 right-0 bottom-0 left-16 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-lg border border-slate-200 w-[420px] shadow-xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50">
              <h3 className="text-sm font-bold text-slate-900">{editingId ? 'Editar Tipo de Ação' : 'Novo Tipo de Ação'}</h3>
            ...
            <form onSubmit={handleSalvar}>
              <div className="p-5 space-y-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wide">Nome do Tipo de Ação *</label>
                  <input type="text" required value={form.nome} onChange={(e) => setForm(prev => ({ ...prev, nome: e.target.value }))}
                    placeholder="Ex: Ajuste Contábil, Reclassificação, Conciliação" className="w-full text-xs p-2 border border-slate-200 rounded-md font-medium text-slate-800 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                </div>
          ...
          <div className="bg-white rounded-lg border border-slate-200 w-[400px] shadow-xl overflow-hidden">
            <div className="p-4 flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500 leading-relaxed">Deseja excluir o tipo de ação <strong className="text-slate-800">"{nomeExcluir}"</strong>?</p>
              </div>
            </div>
            <div className="flex items-center justify-end gap-2 p-3 bg-slate-50 border-t border-slate-100">
