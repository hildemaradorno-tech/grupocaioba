...
import { Users, ChevronRight, ChevronDown, AlertTriangle, X, Loader2, ClipboardCheck, Trash2, Plus, Edit2 } from 'lucide-react'
...
export default function MetasTerceiros() {
  const navigate = useNavigate()
  const [empresas,       setEmpresas]       = useState([])
  const [dados,          setDados]          = useState([])
  const [diasUteis,      setDiasUteis]      = useState({})
  const [filtroEmpresa,  setFiltroEmpresa]  = useSessionState('mter_empresa', '')
  const [filtroAno,      setFiltroAno]      = useSessionState('mter_ano', anoAtual)
  ...
  const [formMeses,        setFormMeses]        = useState(Array.from({length: 12}, (_, i) => ({ mes: i+1, meta_servicos: 0 })))
  ...
  const canEdit   = hasPermission('/metas/pos-vendas/terceiros', 'editar')
  const canDelete = hasPermission('/metas/pos-vendas/terceiros', 'excluir')
  ...
  const abrirVisualizar = (empId, empNome) => {
    const meses = Array.from({ length: 12 }, (_, i) => {
      const mes = i + 1
      return { mes, meta_servicos: Number(getVal(empId, mes, 'meta_servicos')) || 0 }
    })
    setModalEmpId(empId)
  ...
  const handleSalvarModal = async () => {
    ...
    try {
      const emp = empresas.find(e => e.id === modalEmpId)
      const nome = emp ? (emp.empresa_fantasia || emp.nome_empresa) : modalEmpNome
      for (const m of modalFormMeses) {
        await apiService.upsertMetaTerceiros({
          empresa_id: modalEmpId, empresa_nome: nome,
          mes: m.mes, ano: filtroAno,
          meta_servicos:    Number(m.meta_servicos) || 0,
          meta_faturamento: Number(m.meta_servicos) || 0,
  ...
  const abrirModalIncluir = () => {
    setFormEmpresa('')
    setFormMeses(Array.from({length: 12}, (_, i) => ({ mes: i+1, meta_servicos: 0 })))
  ...
  const handleEdit = useCallback(async (empId, empNome, mes, value) => {
    const key = `${empId}_${mes}`
    const newEdit = { meta_servicos: value }
    ...
    try {
      await apiService.upsertMetaTerceiros({
        ...
        meta_servicos:    Number(value) || 0,
        meta_faturamento: Number(value) || 0,
        status: 'AGUARDANDO APROVACAO',
      })
    } catch (err) { setError(err.message || String(err)) }
  ...
  const handleZerar = async () => {
    ...
    try { await apiService.deleteMetasTerceirosEmpresa(modalZerar.empId, filtroAno); await loadAll() }
    catch (err) { setError(err.message || String(err)) }
  ...
  const grupoMeses = useMemo(() =>
    Array.from({ length: 12 }, (_, i) =>
      empList.reduce((s, emp) => s + (Number(getVal(emp.id, i + 1, 'meta_servicos')) || 0), 0)
    ...
    <div className="flex flex-col h-full p-6 gap-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Metas - Terceiros</h1>
              ...
              {loading ? (
                ...
                <>
                  ...
                  {grupoAberto && empList.map(emp => {
                    ...
                    const empMeses = Array.from({ length: 12 }, (_, i) => Number(getVal(empId, i + 1, 'meta_servicos')) || 0)
                    const empAnual = empMeses.reduce((s, v) => s + v, 0)

                    const empRows    = dados.filter(r => r.empresa_id === empId)
                    const hasData    = empMeses.some(v => v > 0)
                    const allApproved = hasData && empRows.length > 0 && empRows.every(r =>
                      cellState(Number(r.meta_servicos) || 0, r.meta_aprovada) === 'ok'
                    )
                                ...
                                <tr className="cursor-pointer bg-slate-100 hover:bg-slate-200 transition-colors"
                                  ...
                                  <td className="px-3 py-1.5 sticky left-0 bg-slate-100 z-10 whitespace-nowrap">
                                    <div className="flex items-center gap-2 pl-12">
                                      ...
                                      <span className="font-semibold text-slate-700">Terceiros</span>
                                    </div>
                                  </td>
                                ...
                                <tr className="cursor-pointer bg-white border-b border-slate-100 hover:bg-slate-50 transition-colors"
                                  ...
                                  <td className="px-3 py-1.5 sticky left-0 bg-white z-10 whitespace-nowrap">
                                    <div className="flex items-center gap-2 pl-16">
                                      ...
                                      <span className="font-semibold text-slate-600">Terceiros</span>
                                    </div>
                                  </td>
                              ...
                              {dOpen && sOpen && bOpen && cOpen && (
                                <tr className="bg-amber-50 border-b border-amber-100">
                                  ...
                                  <td className="px-2 py-1.5 text-center">
                                    {hasData && (
                                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${allApproved ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                                        {allApproved ? 'Aprovado' : 'Aguard. Distribuição'}
                                      </span>
                                    )}
                                  </td>
                                  <td />
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold text-slate-800">
                  {modoModal === 'visualizar' ? 'Visualizar' : 'Editar'} — Terceiros · {modalEmpNome} · {filtroAno}
                </h2>
              </div>
            ...
            <div className="flex-1 overflow-auto px-6 py-4">
              <div className="overflow-x-auto">
                <table className="text-xs border-separate border-spacing-0 w-full" style={{ minWidth: '900px' }}>
                  <thead>
                    <tr className="bg-slate-50">
                      <th className="px-3 py-2 text-left font-semibold text-slate-500 uppercase border-b border-slate-200 sticky left-0 bg-slate-50 w-40"></th>
                      {MESES_ABR.map(m => <th key={m} className="px-1 py-2 text-center font-semibold text-slate-500 uppercase border-b border-slate-200 w-24">{m}</th>)}
                  ...
                  <tbody>
                    ...
                    <tr className="border-b border-slate-100">
                      <td className="px-3 py-1.5 sticky left-0 bg-indigo-50/50 z-10 whitespace-nowrap font-bold text-indigo-700">Meta Serviços (R$)</td>
        ...
        <div className="fixed top-0 right-0 bottom-0 left-16 bg-black/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-slate-200">
              <h2 className="text-base font-bold text-slate-800">Incluir Empresa — Terceiros {filtroAno}</h2>
            ...
            <div className="overflow-auto flex-1 px-6 py-4">
              <div className="mb-4 max-w-xs">
                <label className={LBL}>Empresa</label>
                <select className={SEL} value={formEmpresa} onChange={e => setFormEmpresa(e.target.value)}>
                  <option value="">Selecione a empresa...</option>
                  {empresasParaIncluir.map(e => <option key={e.id} value={e.id}>{e.empresa_fantasia || e.nome_empresa}</option>)}
                </select>
                {empresasParaIncluir.length === 0 && (
                  <p className="text-xs text-amber-600 mt-1">Todas as empresas já foram adicionadas para {filtroAno}.</p>
                )}
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="text-xs border-separate border-spacing-0 w-full">
                  <thead>
                    <tr className="bg-slate-50">
                      <th className="px-3 py-2 text-left font-semibold text-slate-600 border-b border-slate-200 sticky left-0 bg-slate-50 w-40">Métrica</th>
                      {MESES_ABR.map(m => <th key={m} className="px-1 py-2 text-center font-semibold text-slate-600 border-b border-slate-200 w-24">{m}</th>)}
                      <th className="px-2 py-2 text-center font-semibold text-indigo-700 border-b border-slate-200 w-28 bg-indigo-50">Total Ano</th>
                    </tr>
                  </thead>
                  <tbody>
                    {/* Meta Serviços */}
                      ...
                      {formMeses.map((m, i) => (
                        <td key={i} className="p-1 border-l border-slate-100">
                          <EditCell
                            ...
                            saving={false}
                            onChange={val => setFormMeses(prev => prev.map((x, j) => j === i ? { ...x, meta_servicos: val } : x))}
                          />
                        </td>
                      ))}
                      <td className="px-2 py-1.5 text-right text-xs font-bold text-indigo-800 bg-indigo-100 whitespace-nowrap">
              ...
              {erroInclusao && (
                <div className="flex items-center gap-2 mt-3 bg-red-50 border border-red-200 rounded-lg p-3 text-red-700 text-sm">
                  <AlertTriangle size={14} /> {erroInclusao}
                </div>
              )}
            </div>

            <div className="flex gap-3 px-6 py-4 border-t border-slate-200">
              <button onClick={() => setModalIncluir(false)} className={`${BTN_SEC} flex-1 justify-center`} disabled={salvandoInclusao}>Cancelar</button>
              <button onClick={handleIncluirEmpresa} disabled={salvandoInclusao || !formEmpresa}
                className="inline-flex items-center justify-center gap-2 flex-1 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors disabled:opacity-50">
