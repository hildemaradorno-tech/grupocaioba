...
import { Plus, X, AlertTriangle, ShoppingBag, Eye } from 'lucide-react'
...
export default function MovimentoVenda() {
  const [dados, setDados] = useState([])
  const [agrupamentos, setAgrupamentos] = useState([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)

  const [modalAberto, setModalAberto] = useSessionState('movvnd_modal', false)
  const [modalExcluirAberto, setModalExcluirAberto] = useState(false)
  const [editingId, setEditingId] = useSessionState('movvnd_editid', null)
  const [idExcluir, setIdExcluir] = useState(null)
  const [form, setForm] = useSessionState('movvnd_form', FORM_VAZIO)
  ...
  const loadData = async () => {
    ...
    setError(null)
    try {
      const [movimentosData, agrupamentosData] = await Promise.all([
        apiService.getMovimentoVenda(),
        apiService.getAgrupamentoEmpresas()
      ])
      setDados(movimentosData)
  ...
  const abrirExcluir = (item) => {
    ...
    setForm(prev => ({ ...prev, sigla: item.sigla, tipo_movimento: item.tipo_movimento }))
  ...
  const handleSalvar = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await apiService.updateMovimentoVenda(editingId, payload)
      } else {
        await apiService.createMovimentoVenda(payload)
      }
      await loadData()
      setModalAberto(false)
    } catch (err) {
      alert('Erro ao salvar movimento: ' + (err.message || String(err)))
    }
  }

  const handleConfirmarExclusao = async () => {
    try {
      await apiService.deleteMovimentoVenda(idExcluir)
      await loadData()
    } catch (err) {
      alert('Erro ao excluir movimento: ' + (err.message || String(err)))
    } finally {
  ...
  return (
    <div className="p-6 space-y-4 max-w-screen-xl">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 tracking-tight">Movimento de Venda</h1>
          <p className="text-xs text-slate-500">Cadastre e gerencie as tipologias de faturamento, saídas comerciais e remessas operacionais da holding.</p>
        </div>
        {canEdit && (
          <button
        ...
        <table className="w-full table-fixed text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200 text-slate-400 text-[11px] font-semibold uppercase tracking-wider">
              ...
              <th className="p-3 w-32 text-center">Tipo Operação</th>
          ...
          <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-700">
            {dados.length === 0 ? (
              <tr>
                <td colSpan="4" className="p-6 text-center text-slate-400">Nenhum movimento cadastrado.</td>
              </tr>
            ) : dados.map((item) => (
              <tr key={item.id} className="hover:bg-slate-50/70 transition-colors">
                <td className="p-3 text-center text-slate-900 font-mono font-bold bg-slate-50/30">{item.codigo}</td>
                <td className="p-3 text-center font-bold text-emerald-700 uppercase tracking-wide">
                  <span className="bg-emerald-50/70 px-2 py-0.5 rounded border border-emerald-100">{item.sigla}</span>
                </td>
                <td className="p-3 text-slate-900 font-bold">{item.tipo_movimento}</td>
                <td className="p-3">
          ...
          <div className="bg-white rounded-lg border border-slate-200 w-[460px] shadow-xl overflow-hidden animate-scale-in">
            <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                ...
                {editingId ? 'Editar Movimento de Venda' : 'Incluir Movimento de Venda'}
              </h3>
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="col-span-2 flex flex-col gap-1.5">
                    <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wide">Tipo Operação *</label>
                    <select
                      required
                      name="sigla"
                      value={form.sigla}
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    placeholder="Ex: Venda de peças balcão oficina"
      ...
      {modalVisualizarAberto && itemVisualizado && (
        <div className="fixed top-0 right-0 bottom-0 left-16 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-lg border border-slate-200 w-[440px] shadow-xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2"><Eye className="h-4 w-4 text-slate-500" />Visualizar Movimento de Venda</h3>
            ...
            <div className="p-5 grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-0.5">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Tipo Operação</span>
                <span className="text-xs font-bold text-emerald-700 uppercase">{itemVisualizado.sigla || '-'}</span>
              </div>
              <div className="col-span-2 flex flex-col gap-0.5">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Tipo de Movimento</span>
                <span className="text-xs font-semibold text-slate-800">{itemVisualizado.tipo_movimento || '-'}</span>
              </div>
              <div className="flex flex-col gap-0.5">
            ...
            <div className="p-4 flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                  Confirma a remoção do movimento <strong className="text-slate-800">"{form.sigla} - {form.tipo_movimento}"</strong>? Notas de saídas futuras perderão esta classificação relacional no BI.
                </p>
              </div>
            </div>
            <div className="flex items-center justify-end gap-2 p-3 bg-slate-50 border-t border-slate-100">
