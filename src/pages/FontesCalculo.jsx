...
export default function FontesCalculo() {
  const [dados, setDados] = useState([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)

  const [modalAberto, setModalAberto] = useSessionState('fontecalc_modal', false)
  const [modalExcluirAberto, setModalExcluirAberto] = useState(false)
  const [modalVisualizarAberto, setModalVisualizarAberto] = useState(false)
  const [editingId, setEditingId] = useSessionState('fontecalc_editid', null)
  ...
  const loadData = async () => {
    ...
    setError(null)
    try {
      const fontes = await apiService.getFontesCalculo()
      setDados(fontes)
    } catch (err) {
  ...
  const handleSalvar = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await apiService.updateFonteCalculo(editingId, payload)
      } else {
        await apiService.createFonteCalculo(payload)
      }
      await loadData()
      ...
      setErroModal(msg.includes('duplicate key') || msg.includes('unique')
        ? `Já existe uma Fonte de Cálculo com o código "${form.codigo}".`
        : 'Erro ao salvar: ' + msg)
    }
  }
  ...
  const handleConfirmarExclusao = async () => {
    setErroExcluir(null)
    try {
      await apiService.deleteFonteCalculo(idExcluir)
      await loadData()
      ...
      setErroExcluir(
        msg.includes('foreign key') || msg.includes('violates')
          ? 'Não é possível excluir: existem Base(s) de Cálculo vinculada(s) a esta Fonte.'
          : 'Erro ao excluir: ' + msg
      )
    }
  }
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[760px]">
          ...
          <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-700">
            {dados.length === 0 ? (
              <tr>
                <td colSpan="6" className="p-6 text-center text-slate-400">Nenhuma Fonte de Cálculo cadastrada.</td>
              </tr>
            ) : dados.map((item) => (
          ...
          <div className="bg-white rounded-lg border border-slate-200 w-full max-w-[720px] shadow-xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                ...
                {editingId ? 'Editar Fonte de Cálculo' : 'Incluir Fonte de Cálculo'}
              </h3>
              ...
              <div className="p-5 space-y-4 max-h-[74vh] overflow-y-auto custom-scrollbar">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input type="text" name="coluna_empresa" value={form.coluna_empresa} onChange={handleInputChange} placeholder="Ex: Empresa_Nome" className={`${INP} font-mono`} />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className={LBL}>Coluna Data</label>
                    <input type="text" name="coluna_data" value={form.coluna_data} onChange={handleInputChange} placeholder="Ex: Data_Criacao" className={`${INP} font-mono`} />
                  </div>
                </div>
                  ...
                  <label className={`${LBL} flex items-center gap-1`}>
                    ...
                    <span className="relative group cursor-help">
                      ...
                      <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 text-[10px] text-white bg-slate-700 rounded px-2 py-1.5 leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 text-center normal-case font-normal tracking-normal">
                        Opcional. Coluna com o nome do vendedor/consultor/produtivo, usada só quando a Política de Comissão do funcionário for nível INDIVIDUAL.
                      </span>
                    </span>
                  </label>
