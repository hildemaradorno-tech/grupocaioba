...
import { Plus, X, AlertTriangle, Eye, ClipboardList } from 'lucide-react'
...
export default function ProjTemplates() {
  const [dados, setDados] = useState(() => _cache.dados ?? [])
  const [loading, setLoading] = useState(() => _cache.dados === null)
  const [error, setError] = useState(null)
  const [modalAberto, setModalAberto] = useSessionState('proj_templates_modal', false)
  const [modalExcluirAberto, setModalExcluirAberto] = useState(false)
  const [editingId, setEditingId] = useSessionState('proj_templates_editid', null)
  const [idExcluir, setIdExcluir] = useState(null)
  const [form, setForm] = useSessionState('proj_templates_form', { texto: '', ativo: true })
  const [modalVisualizarAberto, setModalVisualizarAberto] = useState(false)
  const [itemVisualizado, setItemVisualizado] = useState(null)
  const { hasActionOrDefault } = useAuth()
  const canEdit = hasActionOrDefault('projetos/templates', 'editar')
  ...
  const loadDados = async (silent = false) => {
    ...
    try {
      setDados(await apiService.getProjTemplates())
    } catch (err) {
  ...
  const abrirIncluir = () => {
    setEditingId(null)
    setForm({ texto: '', ativo: true })
    setModalAberto(true)
  }

  const abrirEditar = (item) => {
    setEditingId(item.id)
    setForm({ texto: item.texto, ativo: item.ativo })
    setModalAberto(true)
  }

  const abrirExcluir = (item) => {
    setIdExcluir(item.id)
    setForm({ texto: item.texto })
  ...
  const handleSalvar = async (e) => {
    e.preventDefault()
    try {
      if (editingId) await apiService.updateProjTemplate(editingId, form)
      else await apiService.createProjTemplate(form)
      await loadDados(true)
      setModalAberto(false)
    } catch (err) {
      alert('Erro ao salvar template: ' + (err.message || String(err)))
    }
  }

  const handleConfirmarExclusao = async () => {
    try {
      await apiService.deleteProjTemplate(idExcluir)
      await loadDados(true)
    } catch (err) {
      alert('Erro ao excluir template: ' + (err.message || String(err)))
    } finally {
  ...
  if (error) return (
    <div className="p-6">
      <div className="bg-yellow-50 border border-yellow-200 rounded p-6">
        <h2 className="text-lg font-semibold mb-2">Erro ao carregar templates</h2>
  ...
  return (
    <div className="p-6 space-y-4 max-w-screen-md">
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 tracking-tight">Templates de Tarefa</h1>
          <p className="text-xs text-slate-500">Textos de pré-preenchimento disponíveis ao criar ou editar tarefas.</p>
        </div>
        {canEdit && (
          <button onClick={abrirIncluir} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold px-3 py-2 rounded-md shadow-sm transition-colors">
            <Plus className="h-4 w-4" /> Incluir Template
          </button>
        )}
      </div>
        ...
        <table className="w-full table-auto text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200 text-slate-400 text-[11px] font-semibold uppercase tracking-wider">
              <th className="p-3">Texto do Template</th>
          ...
          <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-700">
            {dados.length === 0 ? (
              <tr><td colSpan="3" className="p-6 text-center text-slate-400">Nenhum template cadastrado.</td></tr>
            ) : dados.map(item => (
              <tr key={item.id} className="hover:bg-slate-50/70 transition-colors">
                <td className="p-3 text-slate-900 flex items-start gap-2">
                  <ClipboardList className="h-3.5 w-3.5 text-blue-400 shrink-0 mt-0.5" />
                ...
                <td className="p-3">
                  <PermissionActionButtons menuPath="projetos/templates" onView={() => abrirVisualizar(item)} onEdit={() => abrirEditar(item)} onDelete={() => abrirExcluir(item)} />
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="fixed top-0 right-0 bottom-0 left-16 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-lg border border-slate-200 w-[480px] shadow-xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50">
              <h3 className="text-sm font-bold text-slate-900">{editingId ? 'Editar Template' : 'Incluir Novo Template'}</h3>
            ...
            <form onSubmit={handleSalvar}>
              <div className="p-5 space-y-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wide">Texto do Template *</label>
                  <textarea
                    required
                    rows={4}
                    value={form.texto}
                    onChange={(e) => setForm(prev => ({ ...prev, texto: e.target.value }))}
                    placeholder="Ex: Validação da implantação da política de..."
                    className="w-full text-xs p-2 border border-slate-200 rounded-md font-medium text-slate-800 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 resize-none"
                  />
                </div>
      ...
      {modalVisualizarAberto && itemVisualizado && (
        <div className="fixed top-0 right-0 bottom-0 left-16 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-lg border border-slate-200 w-[480px] shadow-xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2"><Eye className="h-4 w-4 text-slate-500" /> Visualizar Template</h3>
            ...
            <div className="p-5 space-y-3">
              <div className="flex flex-col gap-0.5">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Texto do Template</span>
                <span className="text-xs font-semibold text-slate-800 leading-relaxed">{itemVisualizado.texto || '-'}</span>
              </div>
              <div className="flex flex-col gap-0.5">
          ...
          <div className="bg-white rounded-lg border border-slate-200 w-[400px] shadow-xl overflow-hidden">
            <div className="p-4 flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500 leading-relaxed">Deseja excluir o template <strong className="text-slate-800">"{form.texto}"</strong>?</p>
              </div>
            </div>
            <div className="flex items-center justify-end gap-2 p-3 bg-slate-50 border-t border-slate-100">
