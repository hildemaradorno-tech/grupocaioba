...
import { Plus, X, AlertTriangle, Eye, Building } from 'lucide-react'
...
export default function ProjDepartamentos() {
  const [dados, setDados] = useState(() => _cache.dados ?? [])
  const [loading, setLoading] = useState(() => _cache.dados === null)
  const [error, setError] = useState(null)
  const [modalAberto, setModalAberto] = useSessionState('proj_dept_modal', false)
  const [modalExcluirAberto, setModalExcluirAberto] = useState(false)
  const [editingId, setEditingId] = useSessionState('proj_dept_editid', null)
  ...
  const loadDados = async (silent = false) => {
    ...
    try {
      setDados(await apiService.getProjDepartamentos())
    } catch (err) {
  ...
  const handleSalvar = async (e) => {
    e.preventDefault()
    try {
      if (editingId) await apiService.updateProjDepartamento(editingId, form)
      else await apiService.createProjDepartamento(form)
      clearProjLookups('departamentos')
  ...
  const handleConfirmarExclusao = async () => {
    try {
      await apiService.deleteProjDepartamento(idExcluir)
      clearProjLookups('departamentos')
      await loadDados(true)
    } catch (err) {
    ...
    <div className="p-6 space-y-4 max-w-screen-md">
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500">Cadastre os departamentos utilizados na Gestão de Projetos.</p>
        </div>
        {canEdit && (
          <button onClick={abrirIncluir} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold px-3 py-2 rounded-md shadow-sm transition-colors">
            <Plus className="h-4 w-4" /> Incluir Depto.
          </button>
        )}
      </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
        <table className="w-full table-auto text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-700">
            {dados.length === 0 ? (
              <tr><td colSpan="3" className="p-6 text-center text-slate-400">Nenhum departamento cadastrado.</td></tr>
            ) : dados.map(item => (
              <tr key={item.id} className="hover:bg-slate-50/70 transition-colors">
                <td className="p-3 text-slate-900 font-bold flex items-center gap-2"><Building className="h-3.5 w-3.5 text-blue-500" /> {item.nome}</td>
                <td className="p-3">
                ...
                <td className="p-3">
                  <PermissionActionButtons menuPath="proj-departamentos" onView={() => abrirVisualizar(item)} onEdit={() => abrirEditar(item)} onDelete={() => abrirExcluir(item)} />
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="fixed top-0 right-0 bottom-0 left-16 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-lg border border-slate-200 w-[420px] shadow-xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50">
              <h3 className="text-sm font-bold text-slate-900">{editingId ? 'Editar Departamento' : 'Incluir Novo Depto.'}</h3>
              ...
              <div className="p-5 space-y-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input type="text" required value={form.nome} onChange={(e) => setForm(prev => ({ ...prev, nome: e.target.value }))}
                    placeholder="Ex: Controladoria, Vendas" className="w-full text-xs p-2 border border-slate-200 rounded-md font-medium text-slate-800 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
                </div>
          ...
          <div className="bg-white rounded-lg border border-slate-200 w-[400px] shadow-xl overflow-hidden">
            <div className="p-4 flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500 leading-relaxed">Deseja excluir o departamento <strong className="text-slate-800">"{form.nome}"</strong>?</p>
              </div>
            </div>
            <div className="flex items-center justify-end gap-2 p-3 bg-slate-50 border-t border-slate-100">
