...
import { Plus, X, AlertTriangle, Eye, UserCheck } from 'lucide-react'
...
export default function ProjResponsaveis() {
  ...
  const [form, setForm] = useSessionState('proj_resp_form', { nome: '', ativo: true })
    ...
    <div className="p-6 space-y-4 max-w-screen-md">
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500">Cadastre os responsáveis utilizados na Gestão de Projetos.</p>
        </div>
        {canEdit && (
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
        <table className="w-full table-auto text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-700">
            {dados.length === 0 ? (
              <tr><td colSpan="3" className="p-6 text-center text-slate-400">Nenhum responsável cadastrado.</td></tr>
            ) : dados.map(item => (
              <tr key={item.id} className="hover:bg-slate-50/70 transition-colors">
                <td className="p-3 text-slate-900 font-bold flex items-center gap-2"><UserCheck className="h-3.5 w-3.5 text-blue-500" /> {item.nome}</td>
                <td className="p-3">
