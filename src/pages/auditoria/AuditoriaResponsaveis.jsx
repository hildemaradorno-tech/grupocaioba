...
import { Plus, Edit2, Trash2, ShieldAlert, Users, AlertTriangle } from 'lucide-react'
import { apiService } from '../../services/api'

const VAZIO = { nome: '', ativo: true }

export default function AuditoriaResponsaveis() {
  ...
  const load = async () => {
    setLoading(true)
    try { setLista(await apiService.getAuditoriaResponsaveis()) }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) await apiService.updateAuditoriaResponsavel(editId, form)
      else await apiService.createAuditoriaResponsavel(form)
  ...
  const handleExcluir = async () => {
    setErro(null)
    try { await apiService.deleteAuditoriaResponsavel(idExcluir); await load() }
    catch (e) { setErro(e.message || String(e)) }
    ...
    <div className="p-6 space-y-5 max-w-3xl">
      <div className="border-b border-slate-200 pb-4">
        <h1 className="text-xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
          <Users className="h-5 w-5 text-indigo-600" /> Responsáveis de Auditoria
        </h1>
        <p className="text-xs text-slate-500 mt-0.5">Cadastro de responsáveis para uso nas auditorias de O.S.</p>
      </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-4">
        <h2 className="text-xs font-bold text-slate-600 uppercase tracking-wider mb-3">
          {editId ? 'Editar Responsável' : 'Novo Responsável'}
        </h2>
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-10 text-center text-xs text-slate-400">Nenhum responsável cadastrado.</div>
        ) : (
          ...
          <div className="bg-white rounded-lg border border-slate-200 w-[400px] shadow-xl overflow-hidden">
            <div className="p-4 flex items-start gap-3">
              ...
              <div className="space-y-1">
                <h3 className="text-sm font-bold text-slate-900">Remover Responsável</h3>
                <p className="text-xs text-slate-500">Confirma a exclusão? O responsável será removido do cadastro.</p>
              </div>
            </div>
